...
export const {changeEmail, changePassword, loginStart, loginFinish} = createActions({
	changeEmail: (email = "") => ({ email }),
	changePassword: (password = "") => ({ password }),
},'LOGIN_START', 'LOGIN_FINISH', { prefix: "janus/login" });
...
const reducer = handleActions({
	...
	[changePassword]: (state, { payload: password }) => (merge(state, password)),
	[combineActions(changeEmail, changePassword)]: (state, msg) => merge(state, validate(state, msg)),
}, defaultState);
