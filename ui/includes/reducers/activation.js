	...
	return (dispatch, getState) => {
		...
		if (getState().activation.submitable) {
			...
			fetch('/profile/api/activate', {
				...
				res.json()
					.then((body) => dispatch(submitFormFinish(body)))
					.then(() => {
						let headers = res.headers;
						if (headers.has('X-Redirect-Location')) {
							window.location = headers.get(
								'X-Redirect-Location'
							);
						}
					});
...
const reducer = handleActions(
	{
		[changeName]: (state, { payload: { name } }) =>
			merge(state, { preferred_name: name }),
		[changePassword]: (state, { payload: { password } }) =>
			merge(state, {
				password: password,
				password_match: password === state.verify_password,
			}),
		[changePasswordVerifier]: (state, { payload: { verifier } }) =>
			merge(state, {
				verify_password: verifier,
				password_match: state.password === verifier,
			}),

		[submitFormStart]: (state) => merge(state, { loading: true }),
		[submitFormFinish]: (state, { payload }) =>
			merge(state, { loading: false, activated: payload.Active }),
		[USER_FOUND]: (state, { payload }) =>
			merge(state, {
				preferred_name:
					payload.profile.preferred_name || state.preferred_name,
			}),
		...
		[combineActions(
			...
			changePasswordVerifier,
			USER_FOUND
		)]: (state, msg) => merge(state, validate(state, msg)),
	},
	defaultState
);
...
const validate = (state, { payload }) => {
	...
	newState.password_match =
		mergeState.password === mergeState.verify_password;
